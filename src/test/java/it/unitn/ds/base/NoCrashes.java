...
class NoCrashes {
  ...
  void sequentialConsistencyOneWriteClient(int coordinator, int n_nodes) throws InterruptedException {
    final TestsSystemWrapper sys = TestsCommons.createTestSystem("sequentialConsistencyOneWriteClient_" + coordinator,
        n_nodes, coordinator);
    ...
    ActorRef writeClient = sys.system.actorOf(
        Client.propsWithListener(sys.client_read_timeout, sys.client_write_timeout,
            Optional.ofNullable(sys.actors.get(0)), null),
        "clientWrite");
    ...
    for (int i = 0; i < sys.actors.size(); i++) {
      ActorRef readClient = sys.system.actorOf(
          Client.propsWithListener(sys.client_read_timeout, sys.client_write_timeout,
              Optional.ofNullable(sys.actors.get(i)), readProbe.getRef()),
